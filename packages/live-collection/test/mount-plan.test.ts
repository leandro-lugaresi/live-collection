...
const plan = (input: {
  ...
  highestPruned?: number | undefined
  lastResync?: number | undefined
}) =>
  planMount({
    collectionLastApplied: input.lastApplied === undefined ? Option.none() : some(input.lastApplied),
    lastIngested: input.lastIngested === undefined ? Option.none() : some(input.lastIngested),
    highestPruned: input.highestPruned === undefined ? Option.none() : some(input.highestPruned),
    lastResyncAt: input.lastResync === undefined ? Option.none() : some(input.lastResync),
  })

describe("planMount", () => {
  it("decides Snapshot at the last-ingested syncId when no last-applied record exists (fresh install / schema bump)", () => {
    const result = plan({ lastIngested: 7 })
  ...
  it("snapshots at zero when nothing was ever ingested either (true cold start)", () => {
    expect(plan({}).decision).toEqual(MountDecision.Snapshot({ at: id(0) }))
  })

  it("decides Snapshot when a resync crossed the last-applied point, at max(lastIngested, resync)", () => {
    const result = plan({ lastApplied: 5, lastIngested: 9, lastResync: 6 })
    expect(result.decision).toEqual(MountDecision.Snapshot({ at: id(9) }))
    const past = plan({ lastApplied: 5, lastIngested: 9, lastResync: 12 })
    expect(past.decision).toEqual(MountDecision.Snapshot({ at: id(12) }))
  })

  it("decides Skip when the collection is already at or past the last-ingested syncId — even if pruning ate history", () => {
    const result = plan({ lastApplied: 9, lastIngested: 9, highestPruned: 20 })
  ...
  it("decides Snapshot when pruning deleted events above the last-applied (the gap is gone)", () => {
    const result = plan({ lastApplied: 5, lastIngested: 9, highestPruned: 6 })
    expect(result.decision).toEqual(MountDecision.Snapshot({ at: id(9) }))
  })

  it("decides Replay from the last-applied when the journal still covers the gap", () => {
    const result = plan({ lastApplied: 5, lastIngested: 9, highestPruned: 3 })
